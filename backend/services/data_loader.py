...
class CheXpertDataLoader:
    ...
    def _process_uncertain_labels(self) -> None:
        """
        Traite les valeurs NaN et -1 dans le dataset d'entraînement
        
        Stratégie CheXpert :
        - NaN (vide) → 0 : Si non mentionné dans le rapport, c'est implicitement négatif
        - -1 (incertain) → GARDER : Essentiel pour l'analyse d'incertitude (Axe 3)
        """
        if self._data is None:
            return
        
        # Remplacer NaN par 0 pour les colonnes de pathologies
        # (pas mentionné = négatif implicite)
        for col in PATHOLOGY_COLUMNS:
            if col in self._data.columns:
                self._data[col] = self._data[col].fillna(0.0)
        
        logger.info("Traitement des labels : NaN → 0, -1 conservés pour analyse d'incertitude")
    
    def _process_uncertain_labels_valid(self) -> None:
        """
        Traite les valeurs NaN et -1 dans le dataset de validation
        
        Même stratégie que pour l'entraînement :
        - NaN → 0 (négatif implicite)
        - -1 → conservé (incertitude)
        """
        if self._valid_data is None:
            return
        ...
        for col in PATHOLOGY_COLUMNS:
            if col in self._valid_data.columns:
                self._valid_data[col] = self._valid_data[col].fillna(0.0)
        
        logger.info("Traitement validation : NaN → 0, -1 conservés")
